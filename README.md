...
- `acidbase hooks` — report whether each repository's ruff pre-commit hook
  agrees with its `uv.lock`, the single source of truth for the ruff version
  (`LOCAL`, `ALIGNED`, `DRIFT`, `NO-RUFF-HOOK`, ...), list any remaining
  remote `rev:` pins, and exit non-zero on drift. Accepts repo paths or
  `--all` to sweep the roots in `config/security_patch.toml`.
...
- `acidbase scaffold` — show a read-only adoption plan for an initialized
  ...
  a verbatim copy of an older acidbase template is reported as `stale` and
  replaced only under `--refresh-baseline`;
...
(uv-lock + ruff + gitleaks, all `repo: local` so the hook file pins no tool
version and ruff resolves from `uv.lock`), the ruff configuration block, and

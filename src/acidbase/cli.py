...
* ``acidbase hooks`` wraps :func:`acidbase.hooks.hooks_command` to report
  whether each repository's ruff pre-commit hook agrees with its ``uv.lock``
  (the single source of truth for the ruff version).
...
from acidbase.hooks import hooks_command
...
main.add_command(hooks_command, name="hooks")

...
class ChangeKind(str, Enum):
    ...
    KEEP = "keep"
    PRESERVE = "preserve"
    # The local file is a verbatim copy of a *previous* acidbase template
    # (templates/superseded.txt), so it is an old baseline, not local policy.
    # Reported only; --refresh-baseline turns it into REFRESH.
    STALE = "stale"
    REFRESH = "refresh"
...
class ScaffoldAction:
    ...
    def changes_disk(self) -> bool:
        ...
        return self.kind in {ChangeKind.ADD, ChangeKind.EXTEND, ChangeKind.REFRESH}
...
class ScaffoldPlan:
    ...
    def apply(self) -> None:
        ...
        for action in self.changes:
            ...
            if action.kind in {ChangeKind.EXTEND, ChangeKind.REFRESH}:
...
def normalized_digest(raw: bytes) -> str | None:
    """Returns the sha256 of UTF-8 text with BOM stripped and CRLF folded to LF."""
    try:
...
def superseded_digests() -> dict[str, frozenset[str]]:
    """Parses templates/superseded.txt into ``{template path: {digest, ...}}``."""
    try:
        text = _template_bytes(PurePosixPath("superseded.txt")).decode("utf-8-sig")
    ...
    for line in text.splitlines():
        ...
        if len(parts) != 2 or not re.fullmatch(r"[0-9a-f]{64}", parts[1]):
            raise click.ClickException(f"Malformed line in templates/superseded.txt: {line!r}")
        table.setdefault(parts[0], set()).add(parts[1])
    return {path: frozenset(digests) for path, digests in table.items()}


def _is_superseded(template_path: PurePosixPath, local: bytes) -> bool:
    """Returns whether ``local`` is a verbatim copy of a previous version of the template."""
    digest = normalized_digest(local)
    return digest is not None and digest in superseded_digests().get(template_path.as_posix(), frozenset())
...
def build_scaffold_plan(
    ...
    *,
    ...
    for source, relative_destination in assets:
        ...
        elif destination.is_file() and _is_superseded(source, local := destination.read_bytes()):
            if refresh_baseline:
                actions.append(
                    ScaffoldAction(
                        relative_destination,
                        ChangeKind.REFRESH,
                        "verbatim copy of a superseded acidbase template",
                        content,
                        local,
                    )
                )
                ...
                actions.append(
                    ScaffoldAction(
                        relative_destination,
                        ChangeKind.STALE,
                        "verbatim copy of a superseded acidbase template; rerun with --refresh-baseline to replace",
                    )
                )
        else:
            actions.append(
...
@click.option(
    "--refresh-baseline",
    is_flag=True,
    help="Also replace files that are verbatim copies of a superseded acidbase template (reported as stale otherwise).",
)
...
def scaffold_command(
    ...
    needed for a new installable project CLI. A file that is a verbatim copy
    of an older acidbase template is reported as stale and replaced only
    under --refresh-baseline.
    """
